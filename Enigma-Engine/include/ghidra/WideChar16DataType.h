 ...
 */
/// \file WideChar16DataType.h
/// \brief Wide-Character (16-bit/UTF16) data type.
/// Translated from: ghidra.program.model.data.WideChar16DataType
#pragma once
...
/**
 * Wide-Character (16-bit/UTF16) data type.
 *
 * In Java this implements both ArrayStringable and DataTypeWithCharset.
 * The C++ port provides the same methods directly on the class; the
 * ArrayStringable::getArrayStringable() and DataTypeWithCharset
 * detection helpers (dynamic_cast) will not match this class.  Callers
 * that need the interface contract can still call the methods by name.
 *
 * Translated from: ghidra.program.model.data.WideChar16DataType
 */
class WideChar16DataType : public BuiltIn {
public:
    static WideChar16DataType& dataType();

    WideChar16DataType(DataTypeManager* dtm = nullptr);

    DataType* clone(DataTypeManager* dtm) const override;

    int getLength() const override;
    bool hasLanguageDependantLength() const override { return false; }
