 ...
 */
/// \file PaddingSettingsDefinition.h
/// \brief Settings definition for setting the padded/unpadded setting
/// Translated from: ghidra.program.model.data.PaddingSettingsDefinition
#pragma once
 ...
 */
class PaddingSettingsDefinition : public EnumSettingsDefinition {
private:
    PaddingSettingsDefinition() = default;

public:
    static PaddingSettingsDefinition& def();

    /// Checks if the current settings are padded or unpadded
    bool isPadded(const Settings* settings) const;

    /// Set true if value should display padded out with zero's
    void setPadded(Settings* settings, bool isPadded);
