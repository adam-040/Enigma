...
class AbstractJavaAnalyzer : public AbstractAnalyzer {
public:
    AbstractJavaAnalyzer(const std::string& name, const std::string& description,
                         AnalyzerType type = AnalyzerType::BYTE_ANALYZER);
    virtual ~AbstractJavaAnalyzer() = default;
...
protected:
    ...
    Address toCpAddr(Program* program, uint64_t offset);
