...
private:
  ...
  int4 createIndex;				///< Deterministic creation order index
...
public:
  ...
  int4 getCreateIndex(void) const { return createIndex; }	///< Get deterministic creation index
  static int4 nextCreateIndex;		///< Global counter for createIndex
...
public:
  /// \brief Comparator (uses HighVariable createIndex for stable deterministic ordering)
  bool operator<(const HighEdge &op2) const {
    if (a == op2.a) return (b < op2.b);
    if (a->getCreateIndex() == op2.a->getCreateIndex()) {
      return (b->getCreateIndex() < op2.b->getCreateIndex());
    }
    return (a->getCreateIndex() < op2.a->getCreateIndex());
  }
