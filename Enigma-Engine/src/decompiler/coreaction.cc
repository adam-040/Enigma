...

{
  ...
  if (alttype == (Datatype *)0) {
    static bool warned = false; if (!warned) { warned = true; std::cerr << "DET_NULL_TEMP_TYPE\n"; }
    return false;
  }
  ...
  Datatype *outTemp = outvn->getTempType();
  if (outTemp == (Datatype *)0) {
    static bool warned = false; if (!warned) { warned = true; std::cerr << "DET_NULL_OUT_TEMP\n"; }
    return false;
  }
  if (0>newtype->typeOrder(*outTemp)) {
#ifdef TYPEPROP_DEBUG
