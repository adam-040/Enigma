...

{
  ...
  if (grpa != grpb)
    return (grpa < grpb);
  // When entries differ (same group), order by address offset, then by size
  if (entry != b.entry) {
    if (entry->getBase() != b.entry->getBase())
      return (entry->getBase() < b.entry->getBase());
    return (entry->getSize() < b.entry->getSize());
  }
